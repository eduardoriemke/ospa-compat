...
const OSPA_NAV_ITEMS = [
  ...
  { key: 'analises',   label: 'Análises',    href: 'analytics.html', icon: 'ti-chart-bar',   roles: ['projetista','coordenador','admin'], needsId: true },
  ...
  if (editorLink) {
    editorLink.addEventListener('click', e => {
      ...
      if (typeof window.ospaOpenEditor === 'function') {
        window.ospaOpenEditor(projeto, userRole);
      } else {
