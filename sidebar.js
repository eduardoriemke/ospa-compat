...
const OSPA_NAV_ITEMS = [
  { key: 'relatorio',  label: 'Relatório',   href: 'projeto.html',    icon: 'ti-file-text',  roles: ['projetista','coordenador','admin'] },
  { key: 'documentos', label: 'Documentos',  href: 'documentos.html', icon: 'ti-folder',      roles: ['projetista','coordenador','admin'] },
  { key: 'analises',   label: 'Análises',    href: 'analytics.html', icon: 'ti-chart-bar',   roles: ['projetista','coordenador','admin'] },
  { key: 'revisao',    label: 'Revisão',     href: 'revisao.html',   icon: 'ti-list-check',  roles: ['projetista','coordenador','admin'] },
  { key: 'divider' },
  // target/rel só no Editor por enquanto — segue abrindo em aba separada, como já é hoje
  { key: 'editor',     label: 'Editor',      href: 'editor.html',    icon: 'ti-edit',        roles: ['coordenador','admin'], target: '_blank' },
...
function ospaRenderSidebar() {
  ...
  const navHtml = OSPA_NAV_ITEMS.map(item => {
    ...
    return `<a href="${item.href}" class="nav-link${active}"${target}><i class="ti ${item.icon}" aria-hidden="true"></i>${item.label}</a>`;
  }).join('');
